...
import edu.esprit.utils.Mailer;
...
public class ManageController implements Initializable {
        ...
        if (controleDeSaisie()) {
            ...
            if (currentRdv == null) {
                if (RdvService.getInstance().add(rdv)) {
                    AlertUtils.makeSuccessNotification("Rdv ajouté avec succés");
                ...
                if (RdvService.getInstance().edit(rdv)) {
                    AlertUtils.makeSuccessNotification("Rdv modifié avec succés");
    ...
    public void afficherCalendrier(ActionEvent actionEvent) {
        try {
            Parent parent = FXMLLoader.load(Objects.requireNonNull(getClass().getResource(Constants.FXML_BACK_CALENDAR)));
    ...
    private boolean controleDeSaisie() {
        ...
        if (debutDP.getValue().isAfter(finDP.getValue())) {
            AlertUtils.makeInformation("Date de fin doit etre superieure a date de debut");
            return false;
        }
        ...
        if (debutDP.getValue().equals(finDP.getValue())) {
            int debMin = Integer.parseInt(debutMinuteTF.getText());
            int debHeure = Integer.parseInt(debutHeureTF.getText());
            int finMin = Integer.parseInt(finMinuteTF.getText());
            int finHeure = Integer.parseInt(finHeureTF.getText());

            if (debHeure > finHeure) {
                AlertUtils.makeInformation("Date de fin doit etre superieure a date de debut");
                return false;
            } else if (debHeure == finHeure) {
                if (debMin >= finMin) {
                    AlertUtils.makeInformation("Date de fin doit etre superieure a date de debut");
                    return false;
                }
